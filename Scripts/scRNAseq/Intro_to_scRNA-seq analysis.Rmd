...
The steps below encompass the standard pre-processing workflow for scRNA-seq data in Seurat. These represent the selection and filtration of cells based on QC metrics (1), data normalization and scaling (2), and the detection of highly variable features (3).
...
```{r}
PercentageFeatureSet(pbmc,pattern="^ATG") -> pbmc$percent.ATG

# Visualize QC metrics as a violin plot
VlnPlot(pbmc, features = c("percent.ATG"), ncol = 1)
```
...
pbmc_UMAP <- RunUMAP(pbmc, dims = 1:10, reduction = "pca", slot = "data", 
                n.neighbors = 100, n.components = 2, metric = "cosine", 
                learning.rate = 1, min.dist = 1, seed.use = 42, reduction.name = "umap")
